...
public static class AssetLoader
{
    ...
    public static Texture2D TextureFromImage(Image image)
    {
        try
        {
            ...
            // Optimize performance by copying pixels directly into a Texture2D via SetData and premultiplying alpha during the copy.
            rgbaImage.ProcessPixelRows(accessor =>
            {
                for (int y = 0; y < accessor.Height; y++)
                {
                    ...
                    for (int x = 0; x < row.Length; x++)
                    {
                        ...
                        colorData[colorIndex++] = alpha switch
                        {
                            0 => Color.Transparent,
                            255 => new Color(pixel.R, pixel.G, pixel.B, alpha),
                            _ => new Color(
