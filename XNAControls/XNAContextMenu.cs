...
public class XNAContextMenu : XNAControl
{
    ...
    protected virtual int DrawItem(int index, Point point)
    {
        ...
        int textY = FontManager.IsSpriteFont(FontIndex)
            ? point.Y + TextVerticalPadding
            : contentTop + Math.Max(0, (contentHeight - FontManager.GetFontVerticalCenteringValue(FontIndex)) / 2);
        ...
        if (item.HintText != null)
        {
            ...
            int hintTextY = FontManager.IsSpriteFont(HintFontIndex)
                ? point.Y + TextVerticalPadding
                : contentTop + Math.Max(0, (contentHeight - FontManager.GetFontVerticalCenteringValue(HintFontIndex)) / 2);
